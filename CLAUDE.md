...
- **Day operations** — `PATCH /api/plan/days/:date` supports `completed: 'false'` and `skipped: 'false'` (undo), `type` changes (rest → run), and all existing field updates. The arrayFilter no longer requires `completed: false` — removed to support undo. `DELETE /api/plan/days/:date` removes a day from the nested array using MongoDB `$pull` with `$[]`.
- **DayRow actions** — Undo button appears on hover for completed/skipped days. Delete button appears on hover for all days. Rest days show a `+ run` button on hover that opens an inline form to convert to a run day. These are hidden in `readonly` mode.
