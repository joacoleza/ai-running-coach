# Security Review — AI Training Coach
...
**Reviewer:** Automated code review (AI Training Coach GSD security audit)
...
### API (`api/`) — 7 vulnerabilities found at review time; 1 remains after `npm audit fix` + forced bumps (2026-04-28)
...
| ~~P1~~ ✅ Fixed | Deps | `npm audit fix` in `api/` and `web/` | Low | `web/` 0 vulns; `@anthropic-ai/sdk` bumped to 0.91.1; `azure-functions-core-tools` bumped to 4.9.0; 1 moderate remains (`brace-expansion` inside azure-functions-core-tools' own bundled node_modules — unfixable until upstream releases a fix) |
